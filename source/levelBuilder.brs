...
Function CreateLevel(levelSet as string, levelId as integer) as object
    this = {number: levelId, gold: 0, status: m.const.LEVEL_STARTUP, const: m.const, loadMap: load_map}
    ...
    this.map = this.LoadMap(levelMap)
    return this
End Function

Function load_map(levelMap as object) as object
    ...
    for y = m.const.TILES_Y-1 to 0 step -1
        for x = m.const.TILES_X-1 to 0 step -1
            ...
            if id = " " 'empty
                ...
                map[x][y].act = m.const.MAP_EMPTY
                map[x][y].bitmap = invalid
                if m.guards.Count() < m.const.MAX_GUARDS
                    map[x][y].guard = true
                    m.guards.Push({x: x, y: y})
                end if
            else if id = "&" 'Player
                map[x][y].base = m.const.MAP_EMPTY
                map[x][y].act = m.const.MAP_EMPTY
